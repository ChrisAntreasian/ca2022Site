<script lang="ts">
  ...
  const { getHeaderHeight, getFooterHeight }: LayoutElemH =
    getContext(contextHeightKey);
  const image = [500, 404, 403].includes($page.status)
    ? `/${$page.status}.jpg`
    : "/500.jpg";
</script>
...
<section
  style={`--min-height: ${(windowHeight - getHeaderHeight() - getFooterHeight()) / rem}rem`}
>
  <div>
    <h3>{$page.status}</h3>
